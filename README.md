...
```cpp
// main.cpp — identical to real CUDA host code
#include <cstdio>
...
int main() {
    ...
    saxpy<<<1, 8>>>(8, 2.0f, d_x, d_y);  // real CUDA syntax
...
# Step 2: Translate PTX → C++, preprocess <<<>>> syntax
python3 tools/ptx2cpp.py kernel.ptx -o kernel_cpu.cpp -H kernel_cpu.h
python3 tools/cuda_preprocess.py main.cpp -o main_cpu.cpp

# Step 3: Compile and run — no GPU needed
g++ -std=c++17 -O2 -Iinclude/compat -Iinclude main_cpu.cpp kernel_cpu.cpp -o saxpy
./saxpy
